...
public class SinisterTalon extends WeaponManager {

    private static final int REQUIRED_AGILITY = 18;
    private static final int AGILITY_BONUS = 4;
    private static final double CRIT_CHANCE_BONUS = 0.12;
    ...
    private static final String WEAPON_NAME = "Sinister Talon";
    private static final String DESCRIPTION = "Sinister Talon: A vicious hooked blade wielded by the Crimson Veil Rogues, greatly enhancing agility and critical strike lethality.";

    private double lastCritBonus = 0;

    public SinisterTalon(String effect) {
        super(WEAPON_NAME, REQUIRED_AGILITY, ATTACK_DAMAGE, effect, WEIGHT);
    }

    public static SinisterTalon createSinisterTalon(Charecter character, String effect) {
        if (character == null) throw new IllegalArgumentException("Character cannot be null.");
        if (character.getGuild() != Guild.CRIMSON_VEIL_ROGUES)
            throw new IllegalArgumentException("Only Crimson Veil Rogues members can wield the Sinister Talon.");
        if (character.getAgility() >= REQUIRED_AGILITY) {
            return new SinisterTalon(effect);
        }
        throw new IllegalArgumentException("Character does not have the required agility to wield the Sinister Talon.");
    }
