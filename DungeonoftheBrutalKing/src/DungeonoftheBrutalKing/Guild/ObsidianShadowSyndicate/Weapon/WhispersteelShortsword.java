...
public class WhispersteelShortsword extends WeaponManager {

    private static final Guild REQUIRED_GUILD = Guild.OBSIDIAN_SHADOW_SYNDICATE;

    private static final String WEAPON_NAME = "WhispersteelShortsword";
    private static final String EFFECT =
            "WhispersteelShortsword: A balanced shortsword that muffles contact and draws clean. " +
            "Increases attack and improves handling via agility. " +
    ...
    private static final int STATUS_PROC_CHANCE_PERCENT = 12;
    ...
    private WhispersteelShortsword() {
        super(WEAPON_NAME, 0, ATTACK_INCREASE, EFFECT, WEIGHT);
    }

    public static WhispersteelShortsword createWhispersteelShortsword(Charecter character) {
        if (character == null) throw new IllegalArgumentException("Character cannot be null.");
        if (character.getGuild() != REQUIRED_GUILD)
            throw new IllegalArgumentException("Only Obsidian Shadow Syndicate members can wield the WhispersteelShortsword.");
        if (character.getAgility() < REQUIRED_AGILITY)
            throw new IllegalArgumentException("Character does not have the required agility to wield the WhispersteelShortsword.");

        return new WhispersteelShortsword();
    }
