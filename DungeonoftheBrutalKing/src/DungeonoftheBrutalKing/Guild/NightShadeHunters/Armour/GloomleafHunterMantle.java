...
public class GloomleafHunterMantle {
    ...
    private static final String ARMOUR_NAME = "GloomleafHunterMantle";
    private static final String DESCRIPTION =
            "GloomleafHunterMantle: A muted, leaf-woven mantle favored by the Night Shade Hunters. " +
            "Grants defense and bolsters vitality and agility for silent movement and decisive shots.";

    private static final int WEIGHT = 5;

    private static final int REQUIRED_VITALITY = 8;
    ...
    public static GloomleafHunterMantle createGloomleafHunterMantle(Charecter character) {
        if (character == null) throw new IllegalArgumentException("Character cannot be null.");
        if (character.getGuild() != REQUIRED_GUILD)
            throw new IllegalArgumentException("Only Night Shade Hunters members can wear the GloomleafHunterMantle.");
        if (character.getVitality() < REQUIRED_VITALITY)
            throw new IllegalArgumentException("Character does not have the required vitality to wear the GloomleafHunterMantle.");
        if (character.getAgility() < REQUIRED_AGILITY)
            throw new IllegalArgumentException("Character does not have the required agility to wear the GloomleafHunterMantle.");

        return new GloomleafHunterMantle();
    }
