...
public class Wolf extends Enemies {
    ...
    public Wolf(int level, int strength, int charisma, int agility, int intelligence, int wisdom, int vitality) {
        super(
            ...
            intelligence,
            wisdom,
            GameSettings.getInstance().getMonsterImagePath() + "Wolf.png",
            false,
            vitality
        );
        this.level = level;
    ...
    public int attack(Character target, MainGameScreen mainGameScreen) {
        ...
        boolean bleedApplied = RandomFactory.gameplayDouble() < 0.15;
        if (bleedApplied) {
    ...
    public String getImagePath() {
        if (getHitPoints() < 8) {
            return GameSettings.getInstance().getMonsterImagePath() + "Wolf_injured.png";
        }
    ...
    public int getExperienceReward() {
        int base = level * 8;
        int offset = (int) ((RandomFactory.gameplayDouble() * (2 * level * 5 + 1)) - (level * 5));
