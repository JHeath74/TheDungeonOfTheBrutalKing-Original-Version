...
public class UmbralRangerCrossbow extends WeaponManager {

    private static final Guild REQUIRED_GUILD = Guild.NIGHT_SHADE_HUNTERS;

    private static final String WEAPON_NAME = "UmbralRangerCrossboy";
    private static final String EFFECT =
            "UmbralRangerCrossboy: A compact, shadow-fitted crossbow reserved for the Night Shade Hunters. " +
    ...
    private UmbralRangerCrossbow() {
        super(WEAPON_NAME, REQUIRED_STRENGTH, ATTACK_INCREASE, EFFECT, WEIGHT);
    }

    public static UmbralRangerCrossbow createUmbralRangerCrossboy(Charecter character) {
        if (character == null) throw new IllegalArgumentException("Character cannot be null.");
        if (character.getGuild() != REQUIRED_GUILD)
            throw new IllegalArgumentException("Only Night Shade Hunters members can wield the UmbralRangerCrossboy.");
        if (character.getStrength() < REQUIRED_STRENGTH)
            throw new IllegalArgumentException("Character does not have the required strength to wield the UmbralRangerCrossboy.");
        if (character.getAgility() < REQUIRED_AGILITY)
            throw new IllegalArgumentException("Character does not have the required agility to wield the UmbralRangerCrossboy.");

        return new UmbralRangerCrossbow();
    }
