...
public class NightshadeRecurveBow extends WeaponManager {

    private static final Guild REQUIRED_GUILD = Guild.NIGHT_SHADE_HUNTERS;

    private static final String WEAPON_NAME = "NightshadeRecurveBow";
    private static final String EFFECT =
            "NightshadeRecurveBow: A recurved bow wrapped in dark cord, reserved for the Night Shade Hunters. " +
            "Increases attack and bolsters agility while wielded.";

    private static final int WEIGHT = 3;

    private static final int REQUIRED_STRENGTH = 6;
    ...
    private NightshadeRecurveBow() {
        super(WEAPON_NAME, REQUIRED_STRENGTH, ATTACK_INCREASE, EFFECT, WEIGHT);
    }

    public static NightshadeRecurveBow createNightshadeRecurveBow(Charecter character) {
        if (character == null) throw new IllegalArgumentException("Character cannot be null.");
        if (character.getGuild() != REQUIRED_GUILD)
            throw new IllegalArgumentException("Only Night Shade Hunters members can wield the NightshadeRecurveBow.");
        if (character.getStrength() < REQUIRED_STRENGTH)
            throw new IllegalArgumentException("Character does not have the required strength to wield the NightshadeRecurveBow.");
        if (character.getAgility() < REQUIRED_AGILITY)
            throw new IllegalArgumentException("Character does not have the required agility to wield the NightshadeRecurveBow.");

        return new NightshadeRecurveBow();
    }
