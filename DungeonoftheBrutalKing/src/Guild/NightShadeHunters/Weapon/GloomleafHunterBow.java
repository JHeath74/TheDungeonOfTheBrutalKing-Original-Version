...
public class GloomleafHunterBow extends WeaponManager {

    private static final Guild REQUIRED_GUILD = Guild.NIGHT_SHADE_HUNTERS;

    private static final String WEAPON_NAME = "GloomleafHunterBow";
    private static final String EFFECT =
            "GloomleafHunterBow: A quiet, leaf-wrapped bow reserved for the Night Shade Hunters. " +
    ...
    public static GloomleafHunterBow createGloomleafHunterBow(Charecter character) {
        ...
        if (character.getStrength() < REQUIRED_STRENGTH)
            throw new IllegalArgumentException("Character does not have the required strength to wield the GloomleafHunterBow.");
        if (character.getAgility() < REQUIRED_AGILITY)
            throw new IllegalArgumentException("Character does not have the required agility to wield the GloomleafHunterBow.");
