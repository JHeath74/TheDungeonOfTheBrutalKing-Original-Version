...
public class DuskwoodTrackerSpear extends WeaponManager {
    ...
    private static final String WEAPON_NAME = "DuskwoodTrackerSpear";
    private static final String EFFECT =
            "DuskwoodTrackerSpear: A bark-toned spear favored by the Night Shade Hunters. " +
    ...
    public static DuskwoodTrackerSpear createDuskwoodTrackerSpear(Charecter character) {
        ...
        if (character.getStrength() < REQUIRED_STRENGTH)
            throw new IllegalArgumentException("Character does not have the required strength to wield the DuskwoodTrackerSpear.");
        if (character.getAgility() < REQUIRED_AGILITY)
            throw new IllegalArgumentException("Character does not have the required agility to wield the DuskwoodTrackerSpear.");
