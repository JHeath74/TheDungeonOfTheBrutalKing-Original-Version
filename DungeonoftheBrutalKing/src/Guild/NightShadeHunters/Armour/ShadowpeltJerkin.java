...
public class ShadowpeltJerkin {

    private static final Guild REQUIRED_GUILD = Guild.NIGHT_SHADE_HUNTERS;

    private static final String ARMOUR_NAME = "ShadowpeltJerkin";
    private static final String DESCRIPTION =
            "ShadowpeltJerkin: A supple, shadow-dyed jerkin lined with treated pelt for the Night Shade Hunters. " +
            "Grants defense and bolsters vitality and agility for silent travel and sudden bursts of speed.";

    private static final int WEIGHT = 6;

    private static final int REQUIRED_VITALITY = 9;
    ...
    public static ShadowpeltJerkin createShadowpeltJerkin(Charecter character) {
        if (character == null) throw new IllegalArgumentException("Character cannot be null.");
        if (character.getGuild() != REQUIRED_GUILD)
            throw new IllegalArgumentException("Only Night Shade Hunters members can wear the ShadowpeltJerkin.");
        if (character.getVitality() < REQUIRED_VITALITY)
            throw new IllegalArgumentException("Character does not have the required vitality to wear the ShadowpeltJerkin.");
        if (character.getAgility() < REQUIRED_AGILITY)
            throw new IllegalArgumentException("Character does not have the required agility to wear the ShadowpeltJerkin.");

        return new ShadowpeltJerkin();
    }
