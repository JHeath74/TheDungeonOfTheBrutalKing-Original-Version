...
public class NightShadeHunters extends JPanel {
    ...
    public NightShadeHunters(boolean isMember) throws IOException, InterruptedException, ParseException {
        ...
        // Show description for non-members or non-evil
        if (!this.isMember || !isEvil(character.getAlignment())) {
            MainGameScreen.getInstance().setMessageTextPane(description);
        }

        // Membership check and join logic
        if (!this.isMember && !inventory.contains("NightShade Hunters Guild Ring")) {
            // Only one guild at a time
            if (character.getCurrentGuild() != null && character.getCurrentGuild() != GuildType.HUNTER) {
            ...
            if (choice == JOptionPane.YES_OPTION) {
                ...
                character.setCurrentGuild(GuildType.HUNTER);
                JOptionPane.showMessageDialog(
                        this,
    ...
    private void buyGuildSpell() {
        ...
        list.addListSelectionListener(ev -> {
            ...
            Spell s = SpellFactory.createGuildSpell(sel, Guild.NIGHT_SHADE_HUNTERS);
            if (s != null) {
        ...
        buyBtn.addActionListener(ev -> {
            ...
            for (String o : new ArrayList<>(Charecter.getInstance().getGuildSpells())) { if (o != null && o.equalsIgnoreCase(sel)) { JOptionPane.showMessageDialog(dialog, "You already own " + sel + "."); return; } }
            int price = 250;
