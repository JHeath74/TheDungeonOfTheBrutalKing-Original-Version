...
public class BloodshadeStiletto extends WeaponManager {

    private static final int REQUIRED_AGILITY = 15;
    private static final int AGILITY_BONUS = 3;
    private static final double CRIT_CHANCE_BONUS = 0.10;
    ...
    private static final String WEAPON_NAME = "Bloodshade Stiletto";
    private static final String DESCRIPTION = "Bloodshade Stiletto: A wicked blade favored by the Crimson Veil Rogues, granting agility and a chance to strike with deadly precision.";

    // Store the last crit chance bonus applied for correct removal
    private double lastCritBonus = 0;

    public BloodshadeStiletto(String effect) {
        super(WEAPON_NAME, REQUIRED_AGILITY, ATTACK_DAMAGE, effect, WEIGHT);
    }

    public static BloodshadeStiletto createBloodshadeStiletto(Charecter character, String effect) {
        ...
        if (character.getAgility() >= REQUIRED_AGILITY) {
            return new BloodshadeStiletto(effect);
        }
        throw new IllegalArgumentException("Character does not have the required agility to wield the Bloodshade Stiletto.");
    }
