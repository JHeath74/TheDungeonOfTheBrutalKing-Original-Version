...
public class BlessedWarhammer extends WeaponManager {

    private static final int REQUIRED_STRENGTH = 13;
    ...
    public BlessedWarhammer(int requiredStrength, int damage, int weight, String effect) {
        super("Blessed Warhammer", requiredStrength, damage, effect, weight);
    }

    public static BlessedWarhammer createBlessedWarhammer(Charecter character, int damage, String effect) {
        int strength = Integer.parseInt(character.getCharInfo().get(3));
        if (strength >= REQUIRED_STRENGTH) {
            return new BlessedWarhammer(REQUIRED_STRENGTH, damage, WEIGHT, effect);
        }
        throw new IllegalArgumentException("Character does not have the required strength to wield the Blessed Warhammer.");
    }
    ...
    public String getDescription() {
        return "Blessed Warhammer: A heavy warhammer blessed by priests, especially effective against fiends.";
    }
}
