...
public class ObsidianShadowSyndicate extends JPanel {
    ...
    public ObsidianShadowSyndicate() throws IOException, InterruptedException, ParseException {
        ...
        JButton guildSpellsButton = new JButton("Guild Spells");
        ...
        if (status == GuildMembershipStatus.NOT_MEMBER) {
            ...
            if (!isEvil(character.getAlignment())) {
                ...
                buttonPanel.add(guildSpellsButton);
        ...
        guildSpellsButton.addActionListener(e -> {
            try {
                java.awt.Window owner = SwingUtilities.getWindowAncestor(this);
                Spells.SpellsManager sm = new Spells.SpellsManager();
                GuildSpellsDialog dlg = new GuildSpellsDialog((java.awt.Frame) owner, Charecter.getInstance(), SharedData.Guild.OBSIDIAN_SHADOW_SYNDICATE, sm);
                dlg.setVisible(true);
            } catch (Exception ex) {
                try { showGuildSpells(); } catch (Exception ex2) { ex2.printStackTrace(); }
            }
        });
    ...
    private void showGuildSpells() {
        ...
        if (getGuildSpellsCount() >= maxSpells) {
            // Still allow viewing/selling, but buying should be prevented later.
        }
        ...
        if (possibleOwner instanceof java.awt.Frame) {
            dialog = new javax.swing.JDialog((java.awt.Frame) possibleOwner, "Obsidian Guild Spells", java.awt.Dialog.ModalityType.APPLICATION_MODAL);
        } else if (possibleOwner instanceof java.awt.Dialog) {
            dialog = new javax.swing.JDialog((java.awt.Dialog) possibleOwner, "Obsidian Guild Spells", java.awt.Dialog.ModalityType.APPLICATION_MODAL);
        } else {
            dialog = new javax.swing.JDialog((java.awt.Frame) null, "Obsidian Guild Spells", java.awt.Dialog.ModalityType.APPLICATION_MODAL);
        }
