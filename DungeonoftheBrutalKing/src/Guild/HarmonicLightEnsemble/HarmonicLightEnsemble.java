...
public class HarmonicLightEnsemble extends JPanel {
    ...
    public HarmonicLightEnsemble() throws IOException, InterruptedException, ParseException {
        ...
        buySpellsButton.addActionListener(evt -> {
            try {
                ...
                GuildSpellsDialog dlg = new GuildSpellsDialog((Frame) owner, Charecter.getInstance(), SharedData.Guild.HARMONIC_LIGHT_ENSEMBLE, sm);
                dlg.setVisible(true);
            } catch (Exception ex) {
                buyGuildSpell();
            }
    ...
    // Show guild spell buy/sell dialog. Price fixed at 250 gold, refund is 10% with confirmation.
    private void buyGuildSpell() {
        ...
        if (getGuildSpellsCount() >= maxSpells) {
            // allow viewing/selling but prevent purchases later
        }
