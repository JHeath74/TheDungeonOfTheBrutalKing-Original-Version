...
public class RadiantChordblade extends WeaponManager {

    private static final Guild REQUIRED_GUILD = Guild.HARMONILIC_LIGHT_ENSEMBLE;

    private static final String WEAPON_NAME = "RadiantChordblade";
    private static final String EFFECT =
            "RadiantChordblade: A resonant blade reserved for the Harmonic Light Ensemble. " +
    ...
    private RadiantChordblade() {
        super(WEAPON_NAME, REQUIRED_STRENGTH, ATTACK_INCREASE, EFFECT, WEIGHT);
    }

    public static RadiantChordblade createRadiantChordblade(Charecter character) {
        if (character == null) throw new IllegalArgumentException("Character cannot be null.");
        if (character.getGuild() != REQUIRED_GUILD)
            throw new IllegalArgumentException("Only Harmonic Light Ensemble members can wield the RadiantChordblade.");
        if (character.getStrength() < REQUIRED_STRENGTH)
            throw new IllegalArgumentException("Character does not have the required strength to wield the RadiantChordblade.");
        if (character.getCharisma() < REQUIRED_CHARISMA)
            throw new IllegalArgumentException("Character does not have the required charisma to wield the RadiantChordblade.");

        return new RadiantChordblade();
    }
